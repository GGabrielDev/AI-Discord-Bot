...
from runtime_telemetry import (
    add as telemetry_add,
    bump as telemetry_bump,
    note_source as telemetry_note_source,
    telemetry_session,
)
...
async def run_autonomous_loop(subject, topic, max_iterations=3, depth=3, log_func=None):
    ...
    with telemetry_session(
        f"research:{subject}",
        metadata={"topic": topic, "depth": depth, "max_iterations": max_iterations},
    ):
        ...
        if checkpoint and checkpoint.get("status") == "in_progress":
            ...
            telemetry_bump("research.resumed")
        ...
        for iteration in range(start_iteration, max_iterations + 1):
            telemetry_bump("research.iterations")
            ...
            for q_idx, query in enumerate(current_queries):
                ...
                telemetry_bump("research.queries")
                ...
                telemetry_add("search.results_kept", len(results))
                telemetry_add("search.results_rejected", len(rejected_results))
                if rejected_results:
                    await report(
                ...
                for res in results:
                    ...
                    if await db.has_source(url):
                        ...
                        if probe_result.get("found"):
                            telemetry_bump("cache.probe_resolved")
                            ...
                            continue
                        telemetry_bump("cache.probe_miss")
                        ...
                        telemetry_bump("cache.misses.vectordb_source")
                    ...
                    if len(text) > 300:
                        ...
                        if text_hash in seen_hashes:
                            telemetry_bump("url.skipped")
                            telemetry_bump("url.skipped.duplicate_content")
                        ...
                        # Store granular source text adaptively so low-value raw data does not dominate long-term storage.
                        if kept_raw_chunks:
                        ...
                        telemetry_note_source(
                            url,
                            summary_chunks=len(summary_chunks),
                            raw_chunks=len(raw_chunks),
                            raw_chunks_kept=len(kept_raw_chunks),
                            raw_storage_tier=raw_policy.get("raw_storage_tier", ""),
                            raw_words=len(compressed_raw.split()),
                        )
                        telemetry_add("chunks.summary", len(summary_chunks))
                        telemetry_add("chunks.raw_planned", len(raw_chunks))
                        telemetry_add("chunks.raw_kept", len(kept_raw_chunks))
                        if not kept_raw_chunks:
                            telemetry_bump("raw.summary_only_sources")
                        elif len(kept_raw_chunks) < len(raw_chunks):
                            telemetry_bump("raw.capped_sources")
