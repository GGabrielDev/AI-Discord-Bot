...
_search_query_cache = TTLCache(ttl_seconds=300, max_entries=64)
_replan_cache = TTLCache(ttl_seconds=180, max_entries=32)
_decompose_cache = TTLCache(ttl_seconds=300, max_entries=32)
...
async def generate_search_queries(topic: str, num_queries: int = 3) -> list[str]:
    ...
    cache_key = (topic.strip(), num_queries)
    cached_queries = _search_query_cache.get(cache_key)
    if cached_queries is not None:
        telemetry_bump("cache.hits")
        telemetry_bump("cache.hits.planner_queries")
        print(f"[Planner] Cache hit for search plan: '{topic}'")
        return cached_queries
    telemetry_bump("cache.misses")
    telemetry_bump("cache.misses.planner_queries")

    async def produce_queries() -> list[str]:
        llm = LocalLLM()
    ...
    return await _search_query_cache.get_or_set(cache_key, produce_queries)
...
async def evaluate_and_replan(subject: str, existing_knowledge: list[str], stats: dict, num_queries: int = 3) -> tuple[list[str], str]:
    ...
    cache_key = (
        subject.strip(),
        num_queries,
        _fingerprint(existing_knowledge),
        _fingerprint(stats),
    )
    cached_replan = _replan_cache.get(cache_key)
    if cached_replan is not None:
        telemetry_bump("cache.hits")
        telemetry_bump("cache.hits.planner_replan")
        print(f"[Planner] Cache hit for re-plan: '{subject}'")
        return tuple(cached_replan)
    telemetry_bump("cache.misses")
    telemetry_bump("cache.misses.planner_replan")

    async def produce_replan() -> tuple[list[str], str]:
        llm = LocalLLM()
    ...
    return await _replan_cache.get_or_set(cache_key, produce_replan)
...
async def decompose_chain_prompt(prompt: str) -> list[str]:
    ...
    """
    cache_key = _fingerprint(prompt)
    cached_topics = _decompose_cache.get(cache_key)
    if cached_topics is not None:
        telemetry_bump("cache.hits")
        telemetry_bump("cache.hits.planner_decompose")
        print("[Planner] Cache hit for chain decomposition.")
        return cached_topics
    telemetry_bump("cache.misses")
    telemetry_bump("cache.misses.planner_decompose")

    async def produce_topics() -> list[str]:
        llm = LocalLLM()
    ...
    return await _decompose_cache.get_or_set(cache_key, produce_topics)
